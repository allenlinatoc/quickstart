...
class ActiveRecordTest extends DatabaseTest
{
	...
	{
		$venue = AbstractVenue::find(1);
		$venue->name = 'should not throw exception';
		$venue->save();
		$this->assert_equals($venue->name,AbstractVenue::find(1)->name);
	}
	...
	{
		$venue = new AbstractVenue(array('marquee' => 'meme', 'id' => 123));
	...
	{
		$this->assert_true(isset(AbstractVenue::first()->marquee));
	}
	...
	{
		AbstractVenue::$use_custom_get_state_getter = true;

		$event = Event::first();
		$this->assert_equals('ny', $event->venue->state);
		$this->assert_equals('ny', $event->state);

		AbstractVenue::$use_custom_get_state_getter = false;
	}

	public function test_delegate_setter_gh_98()
	{
		AbstractVenue::$use_custom_set_state_setter = true;

		$event = Event::first();
		$event->state = 'MEXICO';
		$this->assert_equals('MEXICO#',$event->venue->state);

		AbstractVenue::$use_custom_set_state_setter = false;
	}
