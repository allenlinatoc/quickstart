...
class ActiveRecordFindTest extends DatabaseTest
{
	...
	{
		$venues = AbstractVenue::all(array('conditions' => array('marquee' => 'Warner Theatre', 'city' => array('Washington','New York'))));
		$this->assert_true(count($venues) >= 1);
	}

	public function test_find_hash_using_alias_with_null()
	{
		$venues = AbstractVenue::all(array('conditions' => array('marquee' => null)));
		$this->assert_equals(0,count($venues));
	}

	public function test_dynamic_finder_using_alias()
	{
		$this->assert_not_null(AbstractVenue::find_by_marquee('Warner Theatre'));
	}
	...
	{
		$venues = AbstractVenue::all(array('select' => 'state', 'group' => 'state'));
	...
	{
		$venues = AbstractVenue::all(array('select' => 'state', 'group' => 'state', 'having' => 'length(state) = 2', 'order' => 'state', 'limit' => 2));
		$this->assert_true(count($venues) > 0);
		$this->assert_sql_has($this->conn->limit('SELECT state FROM venues GROUP BY state HAVING length(state) = 2 ORDER BY state',null,2),AbstractVenue::table()->last_sql);
	}
	...
	{
		$this->assert_equals(2,AbstractVenue::count_by_state('VA'));
		$this->assert_equals(3,AbstractVenue::count_by_state_or_name('VA','Warner Theatre'));
		$this->assert_equals(0,AbstractVenue::count_by_state_and_name('VA','zzzzzzzzzzzzz'));
	}
