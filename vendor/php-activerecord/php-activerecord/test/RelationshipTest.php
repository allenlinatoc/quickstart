...
class RelationshipTest extends DatabaseTest
{
	...
	public function set_up($connection_name=null)
	{
		...
		AbstractVenue::$has_many = array(array('events', 'order' => 'id asc'),array('hosts', 'through' => 'events', 'order' => 'hosts.id asc'));
		AbstractVenue::$has_one = array();
	...
	{
		$this->assert_true($event->$association_name instanceof AbstractVenue);
	...
	public function test_gh_256_eager_loading_three_levels_deep()
	{
		...
		$venue = AbstractVenue::find(2,$conditions);
	...
	{
		AbstractVenue::$has_many = array(array('explicit_class_name', 'class_name' => 'Event', 'order' => 'id asc'));;
		$this->assert_default_has_many($this->get_relationship(), 'explicit_class_name');
	}

	public function test_has_many_with_select()
	{
		AbstractVenue::$has_many[0]['select'] = 'title, type';
	...
	{
		AbstractVenue::$has_many[0]['readonly'] = true;
		$venue = $this->get_relationship();
		$this->assert_default_has_many($venue);
	...
	{
		AbstractVenue::$has_many = array(array('event', 'class_name' => 'Event', 'order' => 'id asc'));
	...
	{
		AbstractVenue::$has_many[0]['conditions'] = "title = 'pr0n @ railsconf'";
		$venue = $this->get_relationship();
		$this->assert_equals(2,$venue->id);
	...
	{
		AbstractVenue::$has_many[0]['conditions'] = "title = 'Yeah Yeah Yeahs'";
		$venue = $this->get_relationship();
		$this->assert_equals(2,$venue->id);
	...
	{
		AbstractVenue::$has_many[0] = array('events',
			'select' => 'type',
			'group'  => 'type',
			'limit'  => 2,
			'offset' => 1);
		AbstractVenue::first()->events;
	...
	{
		$hosts = AbstractVenue::find(2)->hosts;
	...
	public function test_gh16_has_many_through_inside_a_loop_should_not_cause_an_exception()
	{
		...
		foreach (AbstractVenue::all() as $venue)
	...
	public function test_has_many_through_no_association()
	{
		...
		AbstractVenue::$has_many[1] = array('hosts', 'through' => 'blahhhhhhh');
	...
	{
		Event::$belongs_to = array(array('host'));
		AbstractVenue::$has_many[1] = array('hosts', 'through' => 'events', 'select' => 'hosts.*, events.*');
	...
	{
		Event::$belongs_to = array(array('host'));
		AbstractVenue::$has_many[1] = array('hosts', 'through' => 'events', 'conditions' => array('events.title != ?', 'Love Overboard'));
	...
	{
		Event::$belongs_to = array(array('host'));
		AbstractVenue::$has_many[1] = array('hostess', 'through' => 'events', 'source' => 'host');
	...
	{
		Event::$belongs_to = array(array('host'));
		AbstractVenue::$has_one = array(array('invalid_assoc'));
		AbstractVenue::$has_many[1] = array('hosts', 'through' => 'invalid_assoc');
	...
	{
		$x = AbstractVenue::first(array('joins' => array('events')));
		$this->assert_sql_has('INNER JOIN events ON(venues.id = events.venue_id)',AbstractVenue::table()->last_sql);
	}
	...
	{
		AbstractVenue::$has_many = array(array('events'),array('hosts', 'through' => 'events'));
		$venue = AbstractVenue::first();
		$this->assert_true(count($venue->hosts) > 0);
	}
	...
	{
		AbstractVenue::$has_many = array(array('events', 'class_name' => 'Event', 'order' => 'id asc', 'conditions' => array('length(title) = ?', 14)));
		$venues = AbstractVenue::find(array(2, 6), array('include' => 'events'));
	...
	{
		$venues = AbstractVenue::find(array(7, 8), array('include' => 'events'));
	...
	{
		$venues = AbstractVenue::find(array(1,2), array('include' => array('events' => array('host'))));
	...
	{
		$venues = AbstractVenue::find(array(1,2,6,9), array('include' => array('events' => array('host'))));
